...
public class C2S_ToggleBlockRelayPacket {
    ...
    public C2S_ToggleBlockRelayPacket(FriendlyByteBuf buf) {
        this.pos = buf == null ? BlockPos.ZERO : buf.readBlockPos();
        this.relayEnabled = buf != null && buf.readBoolean();
    }

    public void toBytes(FriendlyByteBuf buf) {
        if (buf == null) return;
        buf.writeBlockPos(pos == null ? BlockPos.ZERO : pos);
        buf.writeBoolean(relayEnabled);
    }
