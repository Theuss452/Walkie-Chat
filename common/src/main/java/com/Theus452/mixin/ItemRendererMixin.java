...
public abstract class ItemRendererMixin {
    ...
    private BakedModel useWalkieTalkie3DModel(BakedModel originalModel, ItemStack stack, ItemTransforms.TransformType transformType) {
        if (stack.is(ModItems.WALKIETALKIE.get()) && transformType != ItemTransforms.TransformType.GUI) {
            return this.itemModelShaper.getModelManager().getModel(
                    new ModelResourceLocation(new ResourceLocation(WalkieTalkieMod.MOD_ID, "item/walkie_talkie3d"), "inventory")
            );
        }
